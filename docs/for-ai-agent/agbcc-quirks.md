...
- The type does not have to be a width change: a `void*` parameter assigned to a typed local is not coalesced either, so the two live in separate callee-saved registers. `Hazard_OnHit` keeps `p->hp` on the parameter's register and `p->damageTimer` / `&p->sprite` on the copy's; declaring the third parameter as `Cactus* p` gives one register and one fewer, and `void* owner` with `Cactus* p = owner;` — the shape `HitboxData.fn` actually has — matched.
