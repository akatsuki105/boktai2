...
export const IncludeMacro = `.include "asm/macros.inc"`;
export const Align4 = ".balign 4, 0";
export const Rodata = ".section .rodata";
...
export const dumpU16array = (arr: number[] | Uint16Array, indent: string = "", columns: number = 16): string => {
  if (arr instanceof Uint16Array) arr = Array.from(arr);
  return dumpDataArray(arr, 16, indent, columns);
};

export const dumpU32array = (arr: number[] | Uint32Array, indent: string = "", columns: number = 8): string => {
  if (arr instanceof Uint32Array) arr = Array.from(arr);
