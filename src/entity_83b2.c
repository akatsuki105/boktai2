...
typedef struct {
  ...
  Vec3 pos;             // 0x1C, 根拠: FUN_08009298 が node.q_pos と同じ値を書く
  AuxSprite node;       // 0x24, sprite を指す描画ノード, 根拠: FUN_0822a470/FUN_0822a4e0 に渡している
  AuxSpriteGfx sprite;  // 0x50, 根拠: Video_GetAuxSprite/Video_SetAuxSpritePltt に渡している
} Entity83B2Data;
...
s32 FUN_080091e8(Entity83B2* p, Entity83B2Data* data, s32 idx) {
  ...
  sprite = &data->sprite;
  if (!Video_GetAuxSprite(sprite, SPRITE_HINT_PANEL)) {
    return -1;
  }
  node = &data->node;
  FUN_0822a470(node, sprite, 1);
  node->q_metaspriteIdx = 1;
  data->node.priority = 2;
  return 0;
}
...
NON_MATCH void FUN_08009278(Entity83B2Data* p, s32 plttID) {
  ...
  sprite = &p->sprite;
  if (p->unk_4 == 0) {
    Video_SetAuxSpritePltt(sprite, plttID + 0x4C);
  }
#else
