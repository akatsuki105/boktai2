...
typedef struct {
  ...
  u16 id;              // 0x18, EntityE06A_Init の引数。ヒットボックスの所有者IDとして FUN_0823646c に渡る
  u8 unk_1a[2];        // 0x1A, 読み書きとも未発見 (sprite の4バイト境界合わせ)
  AuxSprite sprite;    // 0x1C, 根拠: FUN_0822a470 / FUN_0822a4e0 に渡される
  AuxSpriteGfx gfx;    // 0x48, 根拠: Video_GetAuxSprite(&gfx, 0x9DA7) / FUN_0822a470 に渡される
...
s32 EntityE06A_Update(EntityE06A* p) {
  FUN_082364c4(&p->hitbox, &p->sprite.q_pos, 0);
  FUN_08236400(&p->hitbox);
  return 0;
}

s32 EntityE06A_Destroy(EntityE06A* p) {
  FUN_0822a4e0(&p->sprite);
  return 0;
}
...
void EntityE06A_SetupSprite(EntityE06A* p) {
  ...
  gfx = &p->gfx;
  Video_GetAuxSprite(gfx, 0x9DA7);
  FUN_0822a470(&p->sprite, gfx, 0);
  z = VM_SeekToKeyword('p');
  if (z != 0) {
    p->sprite.q_pos.x = Script_GetValue();
    p->sprite.q_pos.y = Script_GetValue();
    z = Script_GetValue();
  } else {
    p->sprite.q_pos.x = 0;
    p->sprite.q_pos.y = 0;
...
void EntityE06A_SetupHitbox(EntityE06A* p) {
  ...
  FUN_0823646c(hitbox, p->id, HBFLAG_UNK_13 | HBFLAG_UNK_0, 0, 0x10, &size, &offset);
  FUN_082364f8(hitbox, 100, 0x78, 0, 0x40002, 0x1E);
  FUN_0823651c(hitbox, nop_080cb254, p);
  FUN_082364c4(hitbox, &p->sprite.q_pos, 0);
  FUN_08236400(hitbox);
}
...
void EntityE06A_SetupHitbox2(EntityE06A* p) {
  ...
  size.x = 0x32, size.y = 100, size.z = 0x32;
  offset.x = 0, offset.y = 100, offset.z = 0;
  FUN_0823646c(hitbox, p->id, HBFLAG_UNK_14 | HBFLAG_UNK_0, 0, 0x10, &size, &offset);
  FUN_08236514(hitbox, 10, 2, 1);
  FUN_0823651c(hitbox, nop_080cb258, p);
  FUN_082364c4(hitbox, &p->sprite.q_pos, 0);
  FUN_08236400(hitbox);
}
