...
static_assert(sizeof(EntityC783Slot) == 4);

typedef struct EntityC783 {
  Entity e;                        // 0x000, ENTITY_UNK_11
  void* tilemapFile;               // 0x018, FUN_081db9f4 が GetFile(DIR_TILE_MAP, 0xCD91) を入れ、Video_SetupBGLayout / FUN_0822c398 に渡す
  rgb555* bgPltt;                  // 0x01C, FUN_081db9f4 が GetFile(DIR_BGPLTT, 0x26BB) + 0x14 を入れ、gBgPlttBuffer へ 0x100 色転送する
  ...
  MainSpriteFile* uiSpriteFile;    // 0x380, GetFile(DIR_MAIN_SPRITE, SPRITE_UI_LINK)
  MainSpriteFile* charSpriteFile;  // 0x384, GetFile(DIR_MAIN_SPRITE, SPRITE_DJANGO_SABATA)
  ...
  EntityC783Slot slots[4];         // 0x3AC, 4人分。FUN_081dc38c が毎フレーム gEntity9A9F から埋める
  EntityFunc fn;                   // 0x3BC, EntityC783_Update が呼ぶ。Init は FUN_081dc38c、キャンセルすると FUN_081dc350
  u8* scriptPc;                    // 0x3C0, script keyword 's' の後の FUN_0823d340()。FUN_081dc100 が VM_ParseStringRef に渡す
  ...
  u16 exitScriptID;                // 0x3DA, Init が VM_GetKeywordValue('e', 0)。FUN_081dc2e0 がこれを実行して自分を消す
} EntityC783;
