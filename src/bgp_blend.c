...
typedef struct {
  rgb555* endPltt;         // 0x00, kw: '.s', 補間の終点 (t が最大のときの色)
  rgb555* startPltt;       // 0x04, kw: '.d', 補間の始点, BGP_BLEND_INTERPOLATE がクリアされている場合はこれがそのままコピーされる
  rgb555* outPltt;         // 0x08, kw: '.b', 書き出し先 (常に gBgPlttBuffer 内)
  u8 count;                // 0x0C, kw: '.n' (default 1), 書き出す色数
  u8 timer;                // 0x0D, 毎フレーム +1, t = timer * 32 / 所要フレーム数
  BgPlttBlendFlags flags;  // 0x0E, kw: '.f' (default 0), see BgPlttBlendFlags
  bool8 reverse;           // 0x0F, 0 なら startPltt -> endPltt、 1 なら逆向き
  u8 fadeIn;               // 0x10, kw: '.S' (default: 10), 往路のフレーム数
  u8 fadeOut;              // 0x11, kw: '.D' (default: 10), 復路のフレーム数
