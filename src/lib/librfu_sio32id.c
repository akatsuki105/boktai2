...
static const ALIGNED(4) u16 Sio32ConnectionData[] = {0x494E, 0x544E, 0x4E45, 0x4F44};  // NINTENDO
static const char Sio32IDLib_Var[] = "Sio32ID_030820";

IWRAM_DATA ALIGNED(16) RfuSIO32Id gRfuSIO32Id = {0};  // 0x03005810
...
static void Sio32IDInit(void) {
  ...
  CpuFill32(0, &gRfuSIO32Id, sizeof(RfuSIO32Id));
...
static void Sio32IDIntr(void) {
  ...
  if (gRfuSIO32Id.MS_mode != AGB_CLK_MASTER) REG_SIOCNT |= SIO_ENABLE;
    ...
    if (backup == gRfuSIO32Id.recv_id) {
      if (gRfuSIO32Id.count < 4) {
        ...
        if (gRfuSIO32Id.recv_id == backup) {
          if (regSIODATA32 == (u16)~gRfuSIO32Id.recv_id) ++gRfuSIO32Id.count;
        }
      } else {
  ...
  REG_SIODATA32 = (gRfuSIO32Id.send_id << 16 * (1 - gRfuSIO32Id.MS_mode)) + (gRfuSIO32Id.recv_id << 16 * gRfuSIO32Id.MS_mode);
  if (gRfuSIO32Id.MS_mode == AGB_CLK_MASTER && (gRfuSIO32Id.count != 0 || regSIODATA32 == 0x494e)) {
    for (delay = 0; delay < 600; ++delay);
    if (gRfuSIO32Id.lastId == 0) REG_SIOCNT |= SIO_ENABLE;
  }
}
