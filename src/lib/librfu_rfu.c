...
static_assert(sizeof(struct LLSFStruct) == 16);
...
static u16 rfu_STC_NI_constructLLSF(u8, u8**, NI_COMM*);
...
static void rfu_STC_NI_initSlot_asRecvDataEntity(u8, NI_COMM*);
static void rfu_STC_NI_initSlot_asRecvControllData(u8, NI_COMM*);

IWRAM_DATA RFU_SLOT_STATUS_UNI* gRfuSlotStatusUNI[RFU_CHILD_MAX] = {0};  // 0x030057E0
IWRAM_DATA RFU_SLOT_STATUS_NI* gRfuSlotStatusNI[RFU_CHILD_MAX] = {0};    // 0x030057F0
IWRAM_DATA RFU_LINK_STATUS* gRfuLinkStatus = NULL;                       // 0x03005800
IWRAM_DATA struct RfuStatic* gRfuStatic = NULL;                          // 0x03005804
IWRAM_DATA struct RfuFixed* gRfuFixed = NULL;                            // 0x03005808
...
u16 rfu_initializeAPI(u32* APIBuffer, u16 buffByteSize, IntrFunc* sioIntrTable_p, bool8 copyInterruptToRam) {
  ...
  // is in EWRAM?
  if (((uintptr_t)APIBuffer & 0xF000000) == EWRAM && copyInterruptToRam) return ERR_RFU_API_BUFF_ADR;
  // is not 4-byte aligned?
  if ((u32)APIBuffer & 3) return ERR_RFU_API_BUFF_ADR;
  if (copyInterruptToRam) {
    ...
    if (buffByteSize < buffByteSizeMax) return ERR_RFU_API_BUFF_SIZE;
  }
  if (!copyInterruptToRam) {
  ...
  for (i = 0; i < RFU_CHILD_MAX; ++i) {
    gRfuSlotStatusNI[i]->recvBuff = NULL;
    gRfuSlotStatusNI[i]->recvBuffSize = 0;
    gRfuSlotStatusUNI[i]->recvBuff = NULL;
    gRfuSlotStatusUNI[i]->recvBuffSize = 0;
  }
  ...
  while (buffByteSizeMax-- != 0) *dst++ = *src++;
#else
...
static void rfu_STC_clearAPIVariables(void) {
  ...
  CpuFill16(0, gRfuLinkStatus, sizeof(RFU_LINK_STATUS));
  gRfuLinkStatus->watchInterval = 4;
  gRfuStatic->nowWatchInterval = 0;
  gRfuLinkStatus->parent_child = MODE_NEUTRAL;
  rfu_clearAllSlot();
  gRfuStatic->SCStartFlag = 0;
  for (i = 0; i < RFU_CHILD_MAX; ++i) gRfuStatic->cidBak[i] = 0;
  REG_IME = IMEBackup;
}
...
u16 rfu_UNI_PARENT_getDRAC_ACK(u8* ackFlag) {
  ...
  *ackFlag = 0;
  if (gRfuLinkStatus->parent_child != MODE_PARENT) return ERR_MODE_NOT_PARENT;
  buf = rfu_getSTWIRecvBuffer();
  switch (*buf) {
    case 40:
    case 54:
      if (buf[1] == 0)
        *ackFlag = gRfuLinkStatus->connectSlot_flag;
      else
        *ackFlag = buf[4];
      return 0;
    default:
...
void rfu_setTimerInterrupt(u8 timerNo, IntrFunc* timerIntrTable_p) { STWI_init_timer(timerIntrTable_p, timerNo); }

u8* rfu_getSTWIRecvBuffer(void) { return (u8*)gRfuFixed->STWIBuffer; }

void rfu_setMSCCallback(void (*callback)(u16 reqCommandId)) { STWI_set_Callback_S(callback); }
...
static void rfu_STC_REQ_callback(u8 reqCommand, u16 reqResult) {
  ...
  if (gRfuStatic->flags & 8) gRfuFixed->reqCallback(reqCommand, reqResult);
}
...
static void rfu_CB_defaultCallback(u8 reqCommand, u16 reqResult) {
  ...
  if (reqCommand == ID_CLOCK_SLAVE_MS_CHANGE_ERROR_BY_DMA_REQ) {
    if (gRfuStatic->flags & 8) gRfuFixed->reqCallback(reqCommand, reqResult);
    bmSlotFlags = gRfuLinkStatus->connectSlot_flag | gRfuLinkStatus->linkLossSlot_flag;
    for (i = 0; i < RFU_CHILD_MAX; ++i)
      if ((bmSlotFlags >> i) & 1) rfu_STC_removeLinkData(i, 1);
    gRfuLinkStatus->parent_child = MODE_NEUTRAL;
  }
}
...
u16 rfu_getRFUStatus(u8* rfuState) {
  if (gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[0] != 0x93) return ERR_REQ_CMD_ID;
...
u16 rfu_MBOOT_CHILD_inheritanceLinkStatus(void) {
  ...
  // The size of RFU_LINK_STATUS is 180
  checksum = 0;
  for (i = 0; i < 180 / 2; ++i) checksum += *mb_buff_iwram_p++;
  if (checksum != *(u16*)(IWRAM_START + 0xFA)) return 1;
  CpuCopy16((u16*)IWRAM_START, gRfuLinkStatus, sizeof(RFU_LINK_STATUS));
...
static void rfu_CB_stopMode(u8 reqCommand, u16 reqResult) {
  if (reqResult == 0) REG_SIOCNT = SIO_MULTI_MODE;
...
u32 rfu_REQBN_softReset_and_checkID(void) {
  ...
  if (REG_IME == 0) return ERR_ID_CHECK_IME_DISABLE;
  AgbRFU_SoftReset();
  rfu_STC_clearAPIVariables();
  if ((id = AgbRFU_checkID(30)) == 0) REG_SIOCNT = SIO_MULTI_MODE;
  return id;
}
...
static void rfu_CB_reset(u8 reqCommand, u16 reqResult) {
  if (reqResult == 0) rfu_STC_clearAPIVariables();
  rfu_STC_REQ_callback(reqCommand, reqResult);
}
...
void rfu_REQ_configGameData(u8 mbootFlag, u16 serialNo, const u8* gname, const u8* uname) {
  ...
  packet[0] = serialNo;
  packet[1] = serialNo >> 8;
  if (mbootFlag != 0) packet[1] = (serialNo >> 8) | 0x80;
  for (i = 2; i < 15; ++i) packet[i] = *gname++;
  check_sum = 0;
  unameBackup = uname;
  for (i = 0; i < 8; ++i) {
...
static void rfu_CB_configGameData(u8 reqCommand, u16 reqResult) {
  ...
  if (reqResult == 0) {
    ...
    if (gRfuLinkStatus->my.serialNo & 0x8000) {
      ...
      gRfuLinkStatus->my.mboot_flag = 1;
    } else {
      gRfuLinkStatus->my.mboot_flag = 0;
    }
    for (i = 0; i < RFU_GAME_NAME_LENGTH; ++i) gRfuLinkStatus->my.gname[i] = *gname_uname_p++;
    ++gname_uname_p;
    for (i = 0; i < PLAYER_NAME_LENGTH + 1; ++i) gRfuLinkStatus->my.uname[i] = *gname_uname_p++;
  }
  rfu_STC_REQ_callback(reqCommand, reqResult);
}
...
void rfu_REQ_startSearchChild(void) {
  ...
  if (result == 0) {
    if (gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[7] == 0) rfu_STC_clearLinkStatus(MODE_PARENT);
  } else {
...
static void rfu_CB_startSearchChild(u8 reqCommand, u16 reqResult) {
  if (reqResult == 0) gRfuStatic->SCStartFlag = 1;
  rfu_STC_REQ_callback(reqCommand, reqResult);
}

static void rfu_STC_clearLinkStatus(u8 parent_child) {
  u8 i;
  ...
  for (i = 0; i < RFU_CHILD_MAX; ++i) gRfuLinkStatus->strength[i] = 0;
  gRfuLinkStatus->connectCount = 0;
  gRfuLinkStatus->connectSlot_flag = 0;
  gRfuLinkStatus->linkLossSlot_flag = 0;
  gRfuLinkStatus->getName_flag = 0;
}
...
static void rfu_CB_pollAndEndSearchChild(u8 reqCommand, u16 reqResult) {
  if (reqResult == 0) rfu_STC_readChildList();
  if (reqCommand == ID_SC_POLL_REQ) {
    if (gRfuLinkStatus->my.id == 0) {
      STWI_set_Callback_M(rfu_CB_defaultCallback);
      STWI_send_SystemStatusREQ();
      if (STWI_poll_CommandEnd() == 0) gRfuLinkStatus->my.id = *(u16*)&gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket32.data[0];
    }
  } else if (reqCommand == ID_SC_END_REQ) {
    if (gRfuLinkStatus->parent_child == MODE_NEUTRAL) gRfuLinkStatus->my.id = 0;
    gRfuStatic->SCStartFlag = 0;
  }
  rfu_STC_REQ_callback(reqCommand, reqResult);
}
...
static void rfu_STC_readChildList(void) {
  ...
  if (numSlots != 0) {
    ...
    if (STWI_poll_CommandEnd() == 0) {
      ...
      for (i = 0; i < RFU_CHILD_MAX; ++i) true_slots[i] = *data_p++;
    }
  ...
  for (data_p = &gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[4]; numSlots != 0; data_p += 4) {
    bm_slot_id = data_p[2];
    if (bm_slot_id < RFU_CHILD_MAX && !((gRfuLinkStatus->connectSlot_flag >> bm_slot_id) & 1) && !((gRfuLinkStatus->linkLossSlot_flag >> bm_slot_id) & 1)) {
#if LIBRFU_VERSION < 1026
      if (true_slots[bm_slot_id] != 0) ++gRfuStatic->lsFixedCount[bm_slot_id];
      if (gRfuStatic->lsFixedCount[bm_slot_id] >= 4) {
        gRfuStatic->lsFixedCount[bm_slot_id] = 0;
        gRfuLinkStatus->strength[bm_slot_id] = 255;
        gRfuLinkStatus->connectSlot_flag |= 1 << bm_slot_id;
        ++gRfuLinkStatus->connectCount;
        gRfuLinkStatus->partner[bm_slot_id].id = *(u16*)data_p;
        gRfuLinkStatus->partner[bm_slot_id].slot = bm_slot_id;
        gRfuLinkStatus->parent_child = MODE_PARENT;
...
static void rfu_CB_startSearchParent(u8 reqCommand, u16 reqResult) {
  if (reqResult == 0) rfu_STC_clearLinkStatus(MODE_CHILD);
  rfu_STC_REQ_callback(reqCommand, reqResult);
}
...
static void rfu_CB_pollSearchParent(u8 reqCommand, u16 reqResult) {
  if (reqResult == 0) rfu_STC_readParentCandidateList();
  rfu_STC_REQ_callback(reqCommand, reqResult);
}
  ...
  for (i = 0; i < RFU_CHILD_MAX && numSlots != 0; ++i) {
    ...
    if (my_check_sum == check_sum) {
      ...
      if (*(u16*)packet_p & 0x8000)
        ...
        target->mboot_flag = 0;
      packet_p += 2;
      for (j = 0; j < RFU_GAME_NAME_LENGTH; ++j) target->gname[j] = *packet_p++;
      ++packet_p;
      for (j = 0; j < PLAYER_NAME_LENGTH + 1; ++j) target->uname[j] = *packet_p++;
...
void rfu_REQ_startConnectParent(u16 pid) {
  ...
  if (i == RFU_CHILD_MAX) result = ERR_PID_NOT_FOUND;
  if (result == 0) {
...
static void rfu_CB_pollConnectParent(u8 reqCommand, u16 reqResult) {
  ...
  if (reqResult == 0) {
    ...
    if (gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[7] == 0) {
      bm_slot_flag = 1 << slot;
      if (!(bm_slot_flag & gRfuLinkStatus->connectSlot_flag)) {
        gRfuLinkStatus->connectSlot_flag |= bm_slot_flag;
        gRfuLinkStatus->linkLossSlot_flag &= ~bm_slot_flag;
        gRfuLinkStatus->my.id = id;
        ++gRfuLinkStatus->connectCount;
        gRfuLinkStatus->parent_child = MODE_CHILD;
        ...
        for (i = 0; i < RFU_CHILD_MAX; ++i) {
          if (gRfuLinkStatus->partner[i].id == gRfuStatic->tryPid) {
            if (gRfuLinkStatus->findParentCount != 0) {
              ...
              CpuCopy16(&gRfuLinkStatus->partner[i], &target_local, sizeof(rfuTgtData));
              CpuFill16(0, gRfuLinkStatus->partner, sizeof(gRfuLinkStatus->partner));
              gRfuLinkStatus->findParentCount = 0;
            } else {
        ...
        if (i < RFU_CHILD_MAX) {
          CpuCopy16(target_p, &gRfuLinkStatus->partner[slot], sizeof(rfuTgtData));
...
void rfu_REQ_endConnectParent(void) {
  STWI_set_Callback_M(rfu_CB_pollConnectParent);
  STWI_send_CP_EndREQ();
  if (gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[6] < 4) gRfuStatic->linkEmergencyFlag[gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[6]] = 0;
}
...
u16 rfu_syncVBlank(void) {
  ...
  if (gRfuLinkStatus->parent_child == MODE_NEUTRAL) return 0;
  if (gRfuStatic->nowWatchInterval != 0) --gRfuStatic->nowWatchInterval;
  ...
  if (masterSlave != AGB_CLK_SLAVE)
    ...
    gRfuStatic->flags |= 2;
  if (!(gRfuStatic->flags & 4)) return 0;
  if (gRfuStatic->watchdogTimer == 0) {
    gRfuStatic->flags &= 0xFB;
    bmSlotFlag = gRfuLinkStatus->connectSlot_flag | gRfuLinkStatus->linkLossSlot_flag;
    for (i = 0; i < RFU_CHILD_MAX; ++i)
      if ((bmSlotFlag >> i) & 1) rfu_STC_removeLinkData(i, 1);
    gRfuLinkStatus->parent_child = MODE_NEUTRAL;
    return 1;
  }
...
u16 rfu_REQBN_watchLink(u16 reqCommandId, u8* bmLinkLossSlot, u8* linkLossReason, u8* parentBmLinkRecoverySlot) {
  ...
  *parentBmLinkRecoverySlot = 0;
  if (gRfuLinkStatus->parent_child == MODE_NEUTRAL || gSTWIStatus->msMode == 0) return 0;
  if (gRfuStatic->flags & 4) gRfuStatic->watchdogTimer = 360;
  ...
  if ((u8)reqCommandId == ID_DISCONNECTED_AND_CHANGE_REQ) {
    ...
    *linkLossReason = packet_p_2[5];
    if (*linkLossReason == REASON_LINK_LOSS) *bmLinkLossSlot = gRfuLinkStatus->connectSlot_flag;
    reasonMaybe = 2;
  } else {
    if (reqCommandId == 0x0136) {
      newLinkLossFlag = gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[5];
      newLinkLossFlag ^= gRfuLinkStatus->connectSlot_flag;
      *bmLinkLossSlot = newLinkLossFlag & gRfuLinkStatus->connectSlot_flag;
  ...
  if (reqResult == 0) {
    ...
    for (i = 0; i < RFU_CHILD_MAX; ++i) gRfuLinkStatus->strength[i] = *packet_p++;
    to_req_disconnect = 0;
    i = 0;
  } else {
  ...
  for (; i < RFU_CHILD_MAX; ++i) {
    ...
    if (gRfuStatic->lsFixedCount[i] != 0) {
      ...
      if (gRfuLinkStatus->strength[i] <= 15) gRfuLinkStatus->strength[i] = 16;
    }
#endif
    newLinkLossFlag = 1 << i;
    if (reqResult == 0) {
      if (reasonMaybe == 1 && (gRfuLinkStatus->connectSlot_flag & newLinkLossFlag)) {
              ...
              if (gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[7] == 0) {
                ...
                *linkLossReason = REASON_LINK_LOSS;
                ...
                if (++gRfuStatic->linkEmergencyFlag[i] > gRfuStatic->linkEmergencyLimit) {
                  ...
                  STWI_send_DisconnectREQ(gRfuLinkStatus->connectSlot_flag);
                  STWI_poll_CommandEnd();
                  *bmLinkLossSlot |= newLinkLossFlag;
                  *linkLossReason = REASON_LINK_LOSS;
                }
      ...
      if (gRfuLinkStatus->parent_child == MODE_PARENT && gRfuLinkStatus->strength[i] != 0) {
        if (newLinkLossFlag & gRfuLinkStatus->linkLossSlot_flag) {
          if (gRfuLinkStatus->strength[i] > 10) {
            *parentBmLinkRecoverySlot |= newLinkLossFlag;
            gRfuLinkStatus->connectSlot_flag |= newLinkLossFlag;
            gRfuLinkStatus->linkLossSlot_flag &= ~newLinkLossFlag;
            ++gRfuLinkStatus->connectCount;
            gRfuStatic->linkEmergencyFlag[i] = 0;
          } else {
            gRfuLinkStatus->strength[i] = 0;
          ...
          if (!((gRfuLinkStatus->connectSlot_flag | gRfuLinkStatus->linkLossSlot_flag) & newLinkLossFlag)) {
    ...
    connSlotFlag = gRfuLinkStatus->connectSlot_flag;
    to_disconnect = *bmLinkLossSlot;
    to_disconnect &= connSlotFlag;
    if (newLinkLossFlag & to_disconnect) rfu_STC_removeLinkData(i, 0);
  }
...
#endif
  if ((gRfuLinkStatus->connectSlot_flag & bmLinkLossFlag) && gRfuLinkStatus->connectCount != 0) --gRfuLinkStatus->connectCount;
  gRfuLinkStatus->connectSlot_flag &= bmLinkRetainedFlag = ~bmLinkLossFlag;
  gRfuLinkStatus->linkLossSlot_flag |= bmLinkLossFlag;
  if (gRfuLinkStatus->parent_child == MODE_CHILD && gRfuLinkStatus->connectSlot_flag == 0) gRfuLinkStatus->parent_child = MODE_NEUTRAL;
  if (bmDisconnect) {
    CpuFill16(0, &gRfuLinkStatus->partner[bmConnectedPartnerId], sizeof(rfuTgtData));
    gRfuLinkStatus->linkLossSlot_flag &= bmLinkRetainedFlag;
    gRfuLinkStatus->getName_flag &= bmLinkRetainedFlag;
...
void rfu_REQ_disconnect(u8 bmDisconnectSlot) {
  ...
  if ((gRfuLinkStatus->connectSlot_flag | gRfuLinkStatus->linkLossSlot_flag) & bmDisconnectSlot) {
    gRfuStatic->recoveryBmSlot = bmDisconnectSlot;
    if (gRfuLinkStatus->parent_child == MODE_NEUTRAL && gRfuStatic->flags & 0x80) {
      if (gRfuLinkStatus->linkLossSlot_flag & bmDisconnectSlot) rfu_CB_disconnect(48, 0);
    } else if (gRfuStatic->SCStartFlag && (STWI_set_Callback_M(rfu_CB_defaultCallback), STWI_send_SC_EndREQ(), (result = STWI_poll_CommandEnd()) != 0)) {
...
static void rfu_CB_disconnect(u8 reqCommand, u16 reqResult) {
  ...
  if (reqResult == 3 && gRfuLinkStatus->parent_child == MODE_CHILD) {
    STWI_set_Callback_M(rfu_CB_defaultCallback);
    STWI_send_SystemStatusREQ();
    if (STWI_poll_CommandEnd() == 0 && gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[7] == 0) reqResult = 0;
  }
  gRfuStatic->recoveryBmSlot &= gRfuLinkStatus->connectSlot_flag | gRfuLinkStatus->linkLossSlot_flag;
  gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[8] = gRfuStatic->recoveryBmSlot;
  if (reqResult == 0) {
    for (i = 0; i < RFU_CHILD_MAX; ++i) {
      bm_slot_flag = 1 << i;
      if (bm_slot_flag & gRfuStatic->recoveryBmSlot) rfu_STC_removeLinkData(i, 1);
    }
  }
  if ((gRfuLinkStatus->connectSlot_flag | gRfuLinkStatus->linkLossSlot_flag) == 0) gRfuLinkStatus->parent_child = MODE_NEUTRAL;
  rfu_STC_REQ_callback(reqCommand, reqResult);
  if (gRfuStatic->SCStartFlag) {
    STWI_set_Callback_M(rfu_CB_defaultCallback);
    STWI_send_SC_StartREQ();
    reqResult = STWI_poll_CommandEnd();
    if (reqResult != 0) rfu_STC_REQ_callback(ID_SC_START_REQ, reqResult);
  }
}
...
static void rfu_CB_CHILD_pollConnectRecovery(u8 reqCommand, u16 reqResult) {
  ...
  if (reqResult == 0 && gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[4] == 0 && gRfuStatic->recoveryBmSlot) {
    ...
    for (i = 0; i < RFU_CHILD_MAX; ++i) {
      ...
      if (gRfuStatic->recoveryBmSlot & bm_slot_flag & rfuLinkStatus->linkLossSlot_flag) {
        gRfuLinkStatus->connectSlot_flag |= bm_slot_flag;
        gRfuLinkStatus->linkLossSlot_flag &= ~bm_slot_flag;
        ++gRfuLinkStatus->connectCount;
        gRfuStatic->linkEmergencyFlag[i] = 0;
...
bool8 rfu_getMasterSlave(void) {
  ...
  if (masterSlave == AGB_CLK_MASTER) {
    if (gSTWIStatus->sending) {
      if (gSTWIStatus->reqActiveCommand == ID_MS_CHANGE_REQ || gSTWIStatus->reqActiveCommand == ID_DATA_TX_AND_CHANGE_REQ || gSTWIStatus->reqActiveCommand == ID_RESUME_RETRANSMIT_AND_CHANGE_REQ) masterSlave = AGB_CLK_SLAVE;
    }
  }
  return masterSlave;
}
...
void rfu_clearAllSlot(void) {
  ...
  REG_IME = 0;
  for (i = 0; i < RFU_CHILD_MAX; ++i) {
    CpuFill16(0, gRfuSlotStatusNI[i], 2 * sizeof(NI_COMM));
    CpuFill16(0, gRfuSlotStatusUNI[i], sizeof(UNI_SEND) + sizeof(UNI_RECV));
    gRfuLinkStatus->remainLLFrameSize_C[i] = 16;
  }
  gRfuLinkStatus->remainLLFrameSize_P = LLF_P_SIZE;
  gRfuLinkStatus->sendSlot_NI_flag = 0;
  gRfuLinkStatus->recvSlot_NI_flag = 0;
  gRfuLinkStatus->sendSlot_UNI_flag = 0;
  gRfuStatic->recvRenewalFlag = 0;
  REG_IME = IMEBackup;
}

static void rfu_STC_releaseFrame(u8 bm_slot_id, u8 send_recv, NI_COMM* NI_comm) {
  if (!(gRfuStatic->flags & 0x80)) {
    if (send_recv == 0) gRfuLinkStatus->remainLLFrameSize_P += NI_comm->payloadSize;
    gRfuLinkStatus->remainLLFrameSize_P += 3;
  } else {
    if (send_recv == 0) gRfuLinkStatus->remainLLFrameSize_C[bm_slot_id] += NI_comm->payloadSize;
    gRfuLinkStatus->remainLLFrameSize_C[bm_slot_id] += 2;
  }
}
...
u16 rfu_clearSlot(u8 connTypeFlag, u8 slotStatusIndex) {
  ...
  if (slotStatusIndex >= RFU_CHILD_MAX) return ERR_SLOT_NO;
  if (!(connTypeFlag & (TYPE_UNI_SEND | TYPE_UNI_RECV | TYPE_NI_SEND | TYPE_NI_RECV))) return ERR_COMM_TYPE;
  imeBak = REG_IME;
  REG_IME = 0;
    ...
    for (send_recv = 0; send_recv < 2; ++send_recv) {
      ...
      if (send_recv == 0) {
        if (connTypeFlag & TYPE_NI_SEND) {
          ...
          gRfuLinkStatus->sendSlot_NI_flag &= ~NI_comm->bmSlot_org;
        }
      } else {
        if (connTypeFlag & TYPE_NI_RECV) {
          NI_comm = &gRfuSlotStatusNI[slotStatusIndex]->recv;
          gRfuLinkStatus->recvSlot_NI_flag &= ~(1 << slotStatusIndex);
        }
      }
      if (NI_comm != NULL) {
        if (NI_comm->state & SLOT_BUSY_FLAG) {
          ...
          for (i = 0; i < RFU_CHILD_MAX; ++i)
            if ((NI_comm->bmSlot_org >> i) & 1) NI_comm->failCounter = 0;
        }
        CpuFill16(0, NI_comm, sizeof(NI_COMM));
      }
    }
  }
  if (connTypeFlag & TYPE_UNI_SEND) {
    RFU_SLOT_STATUS_UNI* slotStatusUNI = gRfuSlotStatusUNI[slotStatusIndex];

    if (slotStatusUNI->send.state & SLOT_BUSY_FLAG) {
      if (!(gRfuStatic->flags & 0x80))
        gRfuLinkStatus->remainLLFrameSize_P += 3 + (u8)slotStatusUNI->send.payloadSize;
      else
        gRfuLinkStatus->remainLLFrameSize_C[slotStatusIndex] += 2 + (u8)slotStatusUNI->send.payloadSize;
      gRfuLinkStatus->sendSlot_UNI_flag &= ~slotStatusUNI->send.bmSlot;
    }
    CpuFill16(0, &slotStatusUNI->send, sizeof(UNI_SEND));
  }
  if (connTypeFlag & TYPE_UNI_RECV) {
    CpuFill16(0, &gRfuSlotStatusUNI[slotStatusIndex]->recv, sizeof(UNI_RECV));
  }
  REG_IME = imeBak;
  return 0;
}
...
u16 rfu_setRecvBuffer(u8 connType, u8 slotNo, void* buffer, u32 buffSize) {
  if (slotNo >= RFU_CHILD_MAX) return ERR_SLOT_NO;
  if (connType & TYPE_NI) {
    gRfuSlotStatusNI[slotNo]->recvBuff = buffer;
    gRfuSlotStatusNI[slotNo]->recvBuffSize = buffSize;
  } else if (!(connType & TYPE_UNI)) {
    return ERR_COMM_TYPE;
  } else {
    gRfuSlotStatusUNI[slotNo]->recvBuff = buffer;
    gRfuSlotStatusUNI[slotNo]->recvBuffSize = buffSize;
  }
  return 0;
}

u16 rfu_NI_setSendData(u8 bmSendSlot, u8 subFrameSize, const void* src, u32 size) { return rfu_STC_setSendData_org(32, bmSendSlot, subFrameSize, src, size); }
...
u16 rfu_NI_CHILD_setSendGameName(u8 slotNo, u8 subFrameSize) { return rfu_STC_setSendData_org(64, 1 << slotNo, subFrameSize, &gRfuLinkStatus->my.serialNo, 26); }
...
static u16 rfu_STC_setSendData_org(u8 ni_or_uni, u8 bmSendSlot, u8 subFrameSize, const void* src, u32 dataSize) {
  ...
  u8 i;
  u16 imeBak;
  RFU_SLOT_STATUS_UNI* slotStatus_UNI;
  RFU_SLOT_STATUS_NI* slotStatus_NI;

  if (gRfuLinkStatus->parent_child == MODE_NEUTRAL) return ERR_MODE_NOT_CONNECTED;
  if (!(bmSendSlot & 0xF)) return ERR_SLOT_NO;
  if (((gRfuLinkStatus->connectSlot_flag | gRfuLinkStatus->linkLossSlot_flag) & bmSendSlot) != bmSendSlot) return ERR_SLOT_NOT_CONNECTED;
  if (ni_or_uni & 0x10)
    sendSlotFlag = gRfuLinkStatus->sendSlot_UNI_flag;
  else
    sendSlotFlag = gRfuLinkStatus->sendSlot_NI_flag;
  if (sendSlotFlag & bmSendSlot) return ERR_SLOT_BUSY;
  for (bm_slot_id = 0; bm_slot_id < RFU_CHILD_MAX && !((bmSendSlot >> bm_slot_id) & 1); ++bm_slot_id);
  if (gRfuLinkStatus->parent_child == MODE_PARENT)
    llFrameSize_p = &gRfuLinkStatus->remainLLFrameSize_P;
  else if (gRfuLinkStatus->parent_child == MODE_CHILD)
    llFrameSize_p = &gRfuLinkStatus->remainLLFrameSize_C[bm_slot_id];
  frameSize = llsf_struct[gRfuLinkStatus->parent_child].frameSize;
#ifdef UBFIX
  ...
  if (sending || ni_or_uni == 0x40) {
    ...
    slotStatus_NI->send.nowp[0] = &slotStatus_NI->send.dataType;
    slotStatus_NI->send.remainSize = 7;
    slotStatus_NI->send.bmSlot_org = bmSendSlot;
    ...
    for (i = 0; i < WINDOW_COUNT; ++i) {
      slotStatus_NI->send.recv_ack_flag[i] = 0;
      slotStatus_NI->send.n[i] = 1;
    }
    for (bm_slot_id = 0; bm_slot_id < RFU_CHILD_MAX; ++bm_slot_id) {
      do {
        if ((bmSendSlot >> bm_slot_id) & 1) gRfuSlotStatusNI[bm_slot_id]->send.failCounter = 0;
      } while (0);
    }
    gRfuLinkStatus->sendSlot_NI_flag |= bmSendSlot;
#ifdef UBFIX
    if (llFrameSize_p)
#endif
    ...
    gRfuLinkStatus->sendSlot_UNI_flag |= bmSendSlot;
  }
  REG_IME = imeBak;
  return 0;
}
...
u16 rfu_changeSendTarget(u8 connType, u8 slotStatusIndex, u8 bmNewTgtSlot) {
  RFU_SLOT_STATUS_NI* slotStatusNI;
  u16 imeBak;
  u8 i;
  ...
  if (connType == 0x20) {
    ...
    if ((slotStatusNI->send.state & SLOT_BUSY_FLAG) && (slotStatusNI->send.state & SLOT_SEND_FLAG)) {
      ...
      if (!(connType & bmNewTgtSlot)) {
        if (connType) {
          ...
          REG_IME = 0;
          for (i = 0; i < RFU_CHILD_MAX; ++i) {
            if ((connType >> i) & 1) gRfuSlotStatusNI[i]->send.failCounter = 0;
          }
          gRfuLinkStatus->sendSlot_NI_flag &= ~connType;
    ...
    if (connType == 16) {
      ...
      if (gRfuSlotStatusUNI[slotStatusIndex]->send.state != SLOT_STATE_SEND_UNI) return ERR_SLOT_NOT_SENDING;
      for (bmSlot = 0, i = 0; i < RFU_CHILD_MAX; ++i)
        if (i != slotStatusIndex) bmSlot |= gRfuSlotStatusUNI[i]->send.bmSlot;
      if (bmNewTgtSlot & bmSlot) return ERR_SLOT_TARGET;
      imeBak = REG_IME;
      REG_IME = 0;
      gRfuLinkStatus->sendSlot_UNI_flag &= ~gRfuSlotStatusUNI[slotStatusIndex]->send.bmSlot;
      gRfuLinkStatus->sendSlot_UNI_flag |= bmNewTgtSlot;
...
u16 rfu_UNI_changeAndReadySendData(u8 slotStatusIndex, const void* src, u8 size) {
  ...
  if (UNI_send->state != SLOT_STATE_SEND_UNI) return ERR_SLOT_NOT_SENDING;
  if (gRfuLinkStatus->parent_child == MODE_PARENT) {
    frame_p = &gRfuLinkStatus->remainLLFrameSize_P;
    frameEnd = gRfuLinkStatus->remainLLFrameSize_P + (u8)UNI_send->payloadSize;
  } else {
    frame_p = &gRfuLinkStatus->remainLLFrameSize_C[slotStatusIndex];
    frameEnd = gRfuLinkStatus->remainLLFrameSize_C[slotStatusIndex] + (u8)UNI_send->payloadSize;
  }
  if (frameEnd < size) return ERR_SUBFRAME_SIZE;
  imeBak = REG_IME;
  REG_IME = 0;
  UNI_send->src = src;
  *frame_p = frameEnd - size;
  UNI_send->payloadSize = size;
  UNI_send->dataReady_flag = 1;
  REG_IME = imeBak;
  return 0;
}
...
void rfu_UNI_readySendData(u8 slotStatusIndex) {
  if (slotStatusIndex < RFU_CHILD_MAX) {
    if (gRfuSlotStatusUNI[slotStatusIndex]->send.state == SLOT_STATE_SEND_UNI) gRfuSlotStatusUNI[slotStatusIndex]->send.dataReady_flag = 1;
  }
}

void rfu_UNI_clearRecvNewDataFlag(u8 slotStatusIndex) {
  if (slotStatusIndex < RFU_CHILD_MAX) gRfuSlotStatusUNI[slotStatusIndex]->recv.newData_flag = 0;
}

void rfu_REQ_sendData(bool8 clockChangeFlag) {
  if (gRfuLinkStatus->parent_child != MODE_NEUTRAL) {
    if (gRfuLinkStatus->parent_child == MODE_PARENT && !(gRfuLinkStatus->sendSlot_NI_flag | gRfuLinkStatus->recvSlot_NI_flag | gRfuLinkStatus->sendSlot_UNI_flag)) {
      ...
      if (!gRfuLinkStatus->llf_ready_flag) rfu_constructSendLLFrame();
      if (gRfuLinkStatus->llf_ready_flag) {
    ...
    if (clockChangeFlag) {
      if (gRfuLinkStatus->parent_child == MODE_PARENT) {
        if (gSTWIStatus->callbackS != NULL) gSTWIStatus->callbackS(39);
      } else {
...
static void rfu_CB_sendData(UNUSED u8 reqCommand, u16 reqResult) {
  ...
  if (reqResult == 0) {
    for (i = 0; i < RFU_CHILD_MAX; ++i) {
      if (gRfuSlotStatusUNI[i]->send.dataReady_flag) gRfuSlotStatusUNI[i]->send.dataReady_flag = 0;
      NI_comm = &gRfuSlotStatusNI[i]->send;
      if (NI_comm->state == SLOT_STATE_SEND_NULL) {
        rfu_STC_releaseFrame(i, 0, NI_comm);
        gRfuLinkStatus->sendSlot_NI_flag &= ~NI_comm->bmSlot;
        if (NI_comm->dataType == 1) gRfuLinkStatus->getName_flag |= 1 << i;
        NI_comm->state = SLOT_STATE_SEND_SUCCESS;
      }
    }
  }
  gRfuLinkStatus->llf_ready_flag = 0;
  rfu_STC_REQ_callback(ID_DATA_TX_REQ, reqResult);
}

static void rfu_CB_sendData2(UNUSED u8 reqCommand, u16 reqResult) { rfu_STC_REQ_callback(ID_DATA_TX_REQ, reqResult); }
...
static void rfu_constructSendLLFrame(void) {
  ...
  if (gRfuLinkStatus->parent_child != MODE_NEUTRAL && gRfuLinkStatus->sendSlot_NI_flag | gRfuLinkStatus->recvSlot_NI_flag | gRfuLinkStatus->sendSlot_UNI_flag) {
    gRfuLinkStatus->llf_ready_flag = 0;
    pakcketSize = 0;
    llf_p = (u8*)&gRfuFixed->LLFBuffer[1];
    for (i = 0; i < RFU_CHILD_MAX; ++i) {
      currSize = 0;
      if (gRfuSlotStatusNI[i]->send.state & SLOT_BUSY_FLAG) currSize = rfu_STC_NI_constructLLSF(i, &llf_p, &gRfuSlotStatusNI[i]->send);
      if (gRfuSlotStatusNI[i]->recv.state & SLOT_BUSY_FLAG) currSize += rfu_STC_NI_constructLLSF(i, &llf_p, &gRfuSlotStatusNI[i]->recv);
      if (gRfuSlotStatusUNI[i]->send.state == SLOT_STATE_SEND_UNI) currSize += rfu_STC_UNI_constructLLSF(i, &llf_p);
      if (currSize != 0) {
        if (gRfuLinkStatus->parent_child == MODE_PARENT)
...
static u16 rfu_STC_NI_constructLLSF(u8 bm_slot_id, u8** dest_pp, NI_COMM* NI_comm) {
  u16 size;
  u32 frame;
  u8 i;
  u8* frame8_p;
  const struct LLSFStruct* llsf = &llsf_struct[gRfuLinkStatus->parent_child];

  if (NI_comm->state == SLOT_STATE_SENDING) {
    while (NI_comm->nowp[NI_comm->phase] >= (const u8*)NI_comm->src + NI_comm->dataSize) {
      ++NI_comm->phase;
      if (NI_comm->phase == 4) NI_comm->phase = 0;
    }
  }
  if (NI_comm->state & SLOT_RECV_FLAG) {
    size = 0;
  } else if (NI_comm->state == SLOT_STATE_SENDING) {
    if (NI_comm->nowp[NI_comm->phase] + NI_comm->payloadSize > (const u8*)NI_comm->src + NI_comm->dataSize)
      size = (const u8*)NI_comm->src + NI_comm->dataSize - NI_comm->nowp[NI_comm->phase];
    else
  ...
  frame = (NI_comm->state & 0xF) << llsf->slotStateShift | NI_comm->ack << llsf->ackShift | NI_comm->phase << llsf->phaseShift | NI_comm->n[NI_comm->phase] << llsf->nShift | size;
  if (gRfuLinkStatus->parent_child == MODE_PARENT) frame |= NI_comm->bmSlot << 18;
  frame8_p = (u8*)&frame;
  for (i = 0; i < llsf->frameSize; ++i) *(*dest_pp)++ = *frame8_p++;
  if (size != 0) {
    const u8* src = NI_comm->nowp[NI_comm->phase];
  ...
  if (gRfuLinkStatus->parent_child == MODE_PARENT)
    gRfuLinkStatus->llf_ready_flag = 1;
  else
    gRfuLinkStatus->llf_ready_flag |= 1 << bm_slot_id;
...
static u16 rfu_STC_UNI_constructLLSF(u8 bm_slot_id, u8** dest_p) {
  ...
  u8* frame8_p;
  u8 i;
  UNI_SEND* UNI_send = &gRfuSlotStatusUNI[bm_slot_id]->send;

  if (!UNI_send->dataReady_flag || !UNI_send->bmSlot) return 0;
  llsf = &llsf_struct[gRfuLinkStatus->parent_child];
  frame = (UNI_send->state & 0xF) << llsf->slotStateShift | UNI_send->payloadSize;
  if (gRfuLinkStatus->parent_child == MODE_PARENT) frame |= UNI_send->bmSlot << 18;
  frame8_p = (u8*)&frame;
  for (i = 0; i < llsf->frameSize; ++i) *(*dest_p)++ = *frame8_p++;
  src_p = UNI_send->src;
  gRfuFixed->fastCopyPtr(&src_p, dest_p, UNI_send->payloadSize);
  if (gRfuLinkStatus->parent_child == MODE_PARENT)
    gRfuLinkStatus->llf_ready_flag = 16;
  else
    gRfuLinkStatus->llf_ready_flag |= 16 << bm_slot_id;
  return llsf->frameSize + UNI_send->payloadSize;
}

void rfu_REQ_recvData(void) {
  if (gRfuLinkStatus->parent_child != MODE_NEUTRAL) {
    gRfuStatic->commExistFlag = gRfuLinkStatus->sendSlot_NI_flag | gRfuLinkStatus->recvSlot_NI_flag | gRfuLinkStatus->sendSlot_UNI_flag;
  ...
  if (reqResult == 0 && gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[1]) {
    ...
    for (i = 0; i < RFU_CHILD_MAX; ++i) {
      ...
      if (slotStatusNI->recv.state == SLOT_STATE_RECV_LAST && !((gRfuStatic->NIEndRecvFlag >> i) & 1)) {
        ...
        gRfuLinkStatus->recvSlot_NI_flag &= ~NI_comm->bmSlot;
...
static void rfu_STC_PARENT_analyzeRecvPacket(void) {
  ...
  for (bm_slot_id = 0; bm_slot_id < RFU_CHILD_MAX; ++bm_slot_id) {
    ...
    frames32 >>= 5;
    if (frame_counts[bm_slot_id] == 0) gRfuStatic->NIEndRecvFlag |= 1 << bm_slot_id;
  }
...
static void rfu_STC_CHILD_analyzeRecvPacket(void) {
  ...
  packet_p = &gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[8];
  if (frames_remaining == 0) gRfuStatic->NIEndRecvFlag = 15;
  do {
...
static u16 rfu_STC_analyzeLLSF(u8 slot_id, const u8* src, u16 last_frame) {
  ...
  llsf_p = &llsf_struct[~gRfuLinkStatus->parent_child & (MODE_NEUTRAL & MODE_PARENT)];
  if (last_frame < llsf_p->frameSize) return last_frame;
  frames = 0;
  for (i = 0; i < llsf_p->frameSize; ++i) frames |= *src++ << 8 * i;
  ...
  if (llsf_NI.recvFirst == 0) {
    if (gRfuLinkStatus->parent_child == MODE_PARENT) {
      if ((gRfuLinkStatus->connectSlot_flag >> slot_id) & 1) {
        if (llsf_NI.slotState == LCOM_UNI) {
          ...
          for (i = 0; i < RFU_CHILD_MAX; ++i)
            if (((gRfuSlotStatusNI[i]->send.bmSlot >> slot_id) & 1) && ((gRfuLinkStatus->sendSlot_NI_flag >> slot_id) & 1)) break;
          if (i < RFU_CHILD_MAX) rfu_STC_NI_receive_Sender(i, slot_id, &llsf_NI, src);
        }
      }
    } else {
      s32 conSlots = gRfuLinkStatus->connectSlot_flag & llsf_NI.connSlotFlag;
      ...
      if (conSlots) {
        for (i = 0; i < RFU_CHILD_MAX; ++i) {
          if ((conSlots >> i) & 1) {
            ...
            else if ((gRfuLinkStatus->sendSlot_NI_flag >> i) & 1)
...
static void rfu_STC_UNI_receive(u8 bm_slot_id, const struct RfuLocalStruct* llsf_NI, const u8* src) {
  u8* dest;
  u32 size;
  RFU_SLOT_STATUS_UNI* slotStatusUNI = gRfuSlotStatusUNI[bm_slot_id];
  UNI_RECV* UNI_recv = &slotStatusUNI->recv;

  UNI_recv->errorCode = 0;
  if (gRfuSlotStatusUNI[bm_slot_id]->recvBuffSize < llsf_NI->frame) {
    slotStatusUNI->recv.state = SLOT_STATE_RECV_IGNORE;
    UNI_recv->errorCode = ERR_RECV_BUFF_OVER;
  } else {
    if (UNI_recv->dataBlock_flag) {
      if (UNI_recv->newData_flag) {
        UNI_recv->errorCode = ERR_RECV_UNK;
        goto force_tail_merge;
      }
    } else {
      if (UNI_recv->newData_flag) UNI_recv->errorCode = ERR_RECV_DATA_OVERWRITED;
    }
    UNI_recv->state = SLOT_STATE_RECEIVING;
    size = UNI_recv->dataSize = llsf_NI->frame;
    dest = gRfuSlotStatusUNI[bm_slot_id]->recvBuff;
    gRfuFixed->fastCopyPtr(&src, &dest, size);
    UNI_recv->newData_flag = 1;
    UNI_recv->state = 0;
  }
force_tail_merge:
  if (UNI_recv->errorCode) gRfuStatic->recvErrorFlag |= 16 << bm_slot_id;
}

static void rfu_STC_NI_receive_Sender(u8 NI_slot, u8 bm_flag, const struct RfuLocalStruct* llsf_NI, const u8* data_p) {
  NI_COMM* NI_comm = &gRfuSlotStatusNI[NI_slot]->send;
  u16 state = NI_comm->state;
  u8 n = NI_comm->n[llsf_NI->phase];
  u8 i;
  u16 imeBak;

  if ((llsf_NI->slotState == LCOM_NI && state == SLOT_STATE_SENDING) || (llsf_NI->slotState == LCOM_NI_START && state == SLOT_STATE_SEND_START) || (llsf_NI->slotState == LCOM_NI_END && state == SLOT_STATE_SEND_LAST)) {
    if (NI_comm->n[llsf_NI->phase] == llsf_NI->n) NI_comm->recv_ack_flag[llsf_NI->phase] |= 1 << bm_flag;
  }
  if ((NI_comm->recv_ack_flag[llsf_NI->phase] & NI_comm->bmSlot) == NI_comm->bmSlot) {
    NI_comm->n[llsf_NI->phase] = (NI_comm->n[llsf_NI->phase] + 1) & 3;
    NI_comm->recv_ack_flag[llsf_NI->phase] = 0;
    if ((u16)(NI_comm->state + ~SLOT_STATE_SEND_NULL) <= 1) {
      if (NI_comm->state == SLOT_STATE_SEND_START)
        NI_comm->nowp[llsf_NI->phase] += NI_comm->payloadSize;
      else
        NI_comm->nowp[llsf_NI->phase] += NI_comm->payloadSize << 2;
      ...
      switch (NI_comm->remainSize) {
        ...
        case 0:
          ...
          if (NI_comm->state == SLOT_STATE_SEND_START) {
            for (i = 0; i < WINDOW_COUNT; ++i) {
              NI_comm->n[i] = 1;
              NI_comm->nowp[i] = NI_comm->src + NI_comm->payloadSize * i;
            }
  ...
  if (NI_comm->state != state || NI_comm->n[llsf_NI->phase] != n || (NI_comm->recv_ack_flag[llsf_NI->phase] >> bm_flag) & 1) {
    imeBak = REG_IME;
    REG_IME = 0;
...
static void rfu_STC_NI_receive_Receiver(u8 bm_slot_id, const struct RfuLocalStruct* llsf_NI, const u8* data_p) {
  u16 imeBak;
  u32 state_check = 0;
  RFU_SLOT_STATUS_NI* slotStatus_NI = gRfuSlotStatusNI[bm_slot_id];
  NI_COMM* recvSlot = &slotStatus_NI->recv;
  ...
  if (llsf_NI->slotState == LCOM_NI_END) {
    ...
    if (state == SLOT_STATE_RECV_START && !recvSlot->remainSize) rfu_STC_NI_initSlot_asRecvDataEntity(bm_slot_id, recvSlot);
    if (recvSlot->state == SLOT_STATE_RECEIVING) state_check = 1;
    ...
    if (state == SLOT_STATE_RECV_START) {
      ...
      if (slotStatus_NI->recv.state != SLOT_STATE_RECV_START) return;
      state_check = 1;
    }
  }
  if (state_check != 0) {
    if (llsf_NI->n == ((recvSlot->n[llsf_NI->phase] + 1) & 3)) {
      gRfuFixed->fastCopyPtr(&data_p, (u8**)&recvSlot->nowp[llsf_NI->phase], llsf_NI->frame);
      if (recvSlot->state == SLOT_STATE_RECEIVING) recvSlot->nowp[llsf_NI->phase] += 3 * recvSlot->payloadSize;
...
static void rfu_STC_NI_initSlot_asRecvControllData(u8 bm_slot_id, NI_COMM* NI_comm) {
  u8* llFrameSize_p;
  u32 llFrameSize;
  u8 bm_slot_flag;
  ...
  if (NI_comm->state == 0) {
    if (*llFrameSize_p < llFrameSize) {
      ...
      *llFrameSize_p -= llFrameSize;
      NI_comm->nowp[0] = &NI_comm->dataType;
      NI_comm->remainSize = 7;
      NI_comm->ack = 1;
      NI_comm->payloadSize = 0;
      NI_comm->bmSlot = bm_slot_flag;
      NI_comm->state = SLOT_STATE_RECV_START;
      gRfuLinkStatus->recvSlot_NI_flag |= bm_slot_flag;
    }
  }
}

static void rfu_STC_NI_initSlot_asRecvDataEntity(u8 bm_slot_id, NI_COMM* NI_comm) {
  u8 bm_slot_flag, win_id;

  if (NI_comm->dataType == 1) {
    NI_comm->nowp[0] = (void*)&gRfuLinkStatus->partner[bm_slot_id].serialNo;
  } else {
    if (NI_comm->dataSize > gRfuSlotStatusNI[bm_slot_id]->recvBuffSize) {
      bm_slot_flag = 1 << bm_slot_id;
      gRfuStatic->recvErrorFlag |= bm_slot_flag;
      gRfuLinkStatus->recvSlot_NI_flag &= ~bm_slot_flag;
    ...
    NI_comm->nowp[0] = gRfuSlotStatusNI[bm_slot_id]->recvBuff;
  }
  for (win_id = 0; win_id < WINDOW_COUNT; ++win_id) {
    NI_comm->n[win_id] = 0;
    NI_comm->nowp[win_id] = &NI_comm->nowp[0][NI_comm->payloadSize * win_id];
  }
  NI_comm->remainSize = NI_comm->dataSize;
...
static void rfu_NI_checkCommFailCounter(void) {
  ...
  if (gRfuLinkStatus->sendSlot_NI_flag | gRfuLinkStatus->recvSlot_NI_flag) {
    imeBak = REG_IME;
    REG_IME = 0;
    recvRenewalFlag = gRfuStatic->recvRenewalFlag >> 4;
    for (bm_slot_id = 0; bm_slot_id < RFU_CHILD_MAX; ++bm_slot_id) {
      bm_slot_flag = 1 << bm_slot_id;
      if (gRfuLinkStatus->sendSlot_NI_flag & bm_slot_flag && !(gRfuStatic->recvRenewalFlag & bm_slot_flag)) ++gRfuSlotStatusNI[bm_slot_id]->send.failCounter;
      if (gRfuLinkStatus->recvSlot_NI_flag & bm_slot_flag && !(recvRenewalFlag & bm_slot_flag)) ++gRfuSlotStatusNI[bm_slot_id]->recv.failCounter;
    }
    gRfuStatic->recvRenewalFlag = 0;
    REG_IME = imeBak;
  }
