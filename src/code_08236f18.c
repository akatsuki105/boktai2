...
NON_MATCH bool32 FUN_08236fac(AnimState* p, AnimationFile* files, u16 animIdx, u8 variant, AnimPlayFlags flags) {
#ifdef NONMATCHING_C
  Animation* a;
  AnimCmd cmd;
  s32 idx;

  a = &((Animation*)files->body)[animIdx];
  p->cmds = (AnimCmd*)((u8*)files + a->cmdOffset) + variant * a->cmdCount;
  p->speed = 0x40;
...
bool32 FUN_08237040(AnimState* p, AnimationFile* files, u16 animIdx, u8 variant, AnimPlayFlags flags) {
  p->cmdCount = 0;
  return FUN_08236fac(p, files, animIdx, variant, flags);
}

// 1コマの表示時間を変更する。経過 tick を新旧の比率で配分し直すので進行の割合は保たれる, リネーム案: Anim_SetWait
bool32 FUN_08237064(AnimState* p, u16 val) {
  if (p->wait == 0) {
    p->wait = 1;
  }
...
void FUN_08237098(AnimState* p, u16 speed) {
  if (p->speed != speed) {
