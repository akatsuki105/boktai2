...
// テキスト表示まわりの構成
//
//   TextRenderer   文字列を1歩ずつ進めるエンジン。エンティティではなく、下の3つが1つずつ内部に持つ (352B)
//     |            枠も id もフレームも知らず、渡された char* を進めてタグを展開するだけ
//     +- TextBox       画面に1つしかない共有の本文表示領域 (752B, src/entity_0e37.c)
//     |    |           使う側が TextBox_SetRect で設定し直して借りる。顔グラ・送り矢印・オート送り・行ラベルつき
//     |    |           会話だけでなく gameover / title_card / credits / 各種図鑑も使う
//     |    +- TextBoxChoice  TextBox を操作して <ALTER> を選択肢として選ばせる UI (452B, src/textbox_choice.c)
//     |                      自分では描かず、カーソルのスプライトを左右に置く
//     +- TextPanel     各画面が必要なだけ自分で開き、id で持って自分で閉じる追加パネル (452B, src/text_panel.c)
//     |                TextPanelManager が双方向リストで抱え、複数同時に存在できる
//     +- EntityDFC6    未解明 (408B, src/entity_dfc6.c)。offset を毎フレーム進めて DAT_03002CE8 に流す
//
...
typedef struct {
  ...
  u8 finished;       // 0x00F, 0 以外なら TextRenderer_Advance が 1 (終了) を返す
  u8 unk_10;         // 0x010
...
void TextRenderer_SetRect(TextRenderer* p, u8 x, u8 y, u8 width, u8 height);
...
static inline void TextRenderer_GetRect(TextRenderer* r, u32* out) {
  out[0] = r->rectX;
  out[1] = r->rectY;
  out[2] = r->rectW;
  out[3] = r->rectH;
